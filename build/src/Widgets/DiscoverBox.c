...
static void __lambda7_ (GradioDiscoverBox* self);
gboolean gradio_radio_stations_provider_get_isWorking (GradioRadioStationsProvider* self);
static void ___lambda7__gradio_radio_stations_provider_status_changed (GradioRadioStationsProvider* _sender, gpointer self);
static void gradio_discover_box_SearchButton_clicked (GradioDiscoverBox* self);
...
static void _gradio_discover_box_SearchButton_clicked_gtk_button_clicked (GtkButton* _sender, gpointer self);
...
static void __lambda7_ (GradioDiscoverBox* self) {
	...
	if (_tmp2_) {
		...
		_tmp3_ = self->priv->SearchButton;
		gtk_widget_set_sensitive ((GtkWidget*) _tmp3_, FALSE);
		_tmp4_ = self->priv->SearchStack;
		gtk_stack_set_visible_child_name (_tmp4_, "loading");
	} else {
		GtkButton* _tmp5_ = NULL;
		GtkStack* _tmp6_ = NULL;
		_tmp5_ = self->priv->SearchButton;
		gtk_widget_set_sensitive ((GtkWidget*) _tmp5_, TRUE);
		_tmp6_ = self->priv->SearchStack;
		gtk_stack_set_visible_child_name (_tmp6_, "no_results");
	}
}


static void ___lambda7__gradio_radio_stations_provider_status_changed (GradioRadioStationsProvider* _sender, gpointer self) {
	__lambda7_ ((GradioDiscoverBox*) self);
}
...
GradioDiscoverBox* gradio_discover_box_construct (GType object_type, GradioGradioApp** a) {
	...
	g_signal_connect_object (_tmp3_, "status-changed", (GCallback) ___lambda7__gradio_radio_stations_provider_status_changed, self, 0);
	return self;
}
...
static void gradio_discover_box_SearchButton_clicked (GradioDiscoverBox* self) {
	...
	gradio_radio_stations_provider_search_radio_stations (_tmp0_, _tmp2_, GRADIO_SEARCH_BY_NAME, 20, ___lambda8__gasync_ready_callback, g_object_ref (self));
}


static void _gradio_discover_box_SearchButton_clicked_gtk_button_clicked (GtkButton* _sender, gpointer self) {
	gradio_discover_box_SearchButton_clicked ((GradioDiscoverBox*) self);
}
		...
		if (_tmp1_) {
			...
			GtkButton* _tmp24_ = NULL;
			{
				...
				while (TRUE) {
					...
					g_message ("DiscoverBox.vala:72: added");
			...
			_tmp24_ = self->priv->SearchButton;
...
static void gradio_discover_box_class_init (GradioDiscoverBoxClass * klass) {
	...
	gtk_widget_class_bind_template_child_full (GTK_WIDGET_CLASS (klass), "SearchButton", FALSE, GradioDiscoverBox_private_offset + G_STRUCT_OFFSET (GradioDiscoverBoxPrivate, SearchButton));
	gtk_widget_class_bind_template_callback_full (GTK_WIDGET_CLASS (klass), "SearchButton_clicked", G_CALLBACK(_gradio_discover_box_SearchButton_clicked_gtk_button_clicked));
...
static void gradio_discover_box_finalize (GObject* obj) {
	...
	_g_object_unref0 (self->priv->SearchButton);
