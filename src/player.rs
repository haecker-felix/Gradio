...
use std::rc::Rc;
use std::thread;
use rustio::{Station, Client};
use mpris_player::{MprisPlayer, PlaybackStatus, OrgMprisMediaPlayer2Player, Metadata};
use std::sync::Arc;
...
impl Player{
    pub fn new(sender: Sender<Action>) -> Self{
        ...
        let mpris = MprisPlayer::new(
            "Gradio".to_string(),
            "Gradio".to_string(),
            "de.haeckerfelix.Gradio".to_string(),
        );
    ...
    pub fn set_playback(&self, playback: PlaybackState){
        match playback{
            PlaybackState::Playing => { self.playbin.set_state(gstreamer::State::Playing); },
